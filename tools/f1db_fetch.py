...
# Any of these in a Creative Commons deed names a different licence.
...
def licence_check(path):
    ...
    its title is not the Attribution 4.0 International deed, or if it names an
    element (NonCommercial, ShareAlike, NoDerivatives) that CC BY does not
    have. A licence that has changed is a classification decision for
    SOURCE_LICENCE in data/current.py, not something a fetch may decide.
    """
    ...
    if title != LICENCE:
        sys.exit(f"{name} in {path} is titled {title!r}, not {LICENCE!r}: F1DB's licence "
                 f"has changed. Reclassify it in SOURCE_LICENCE (data/current.py) "
                 f"before fetching again; the committed harvest stays under the "
                 f"licence it was received under (docs/UPSTREAM.md)")
    named = [w for w in LICENCE_FORBIDDEN if w in text]
    if named:
        sys.exit(f"{name} in {path} names {', '.join(named)}, which the Attribution 4.0 "
                 f"deed does not: F1DB's licence has changed. Reclassify it in "
                 f"SOURCE_LICENCE (data/current.py) before fetching again")
    return name
