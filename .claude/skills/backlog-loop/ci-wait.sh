...
# none is pending, or "PR N timeout: ..." after ~20 minutes. Empty output
# from gh (API hiccup, checks not yet registered) counts as pending.
...
print(",".join(c.get(k, "?") for k in ["check (3.9)", "check (3.12)", "web"]))' 2>/dev/null)
  case "$out" in
    ""|*pending*|*'?'*) sleep 30 ;;
    *) echo "PR $n core checks: $out"; exit 0 ;;
  esac
done
