...
-- "Current" is read off the data, not typed: season_entries holds the season
-- in progress and nothing else, so MAX(year) over it IS this year and the view
-- does not need editing when the year turns (CR-07). verify.py checks that the
-- table holds exactly one year and that it is meta.current_season.
...
WHERE e.year = (SELECT MAX(year) FROM season_entries)
...
-- season_entries for the same reason v_current_grid uses it - it is the one
-- table that holds this season alone - and NOT MAX(races.year), which is the
-- next season's calendar as soon as one is announced.
CREATE VIEW v_lost_circuits AS
SELECT c.id, c.name, c.country, COUNT(r.id) AS races,
       MIN(r.year) AS first_gp, MAX(r.year) AS last_gp,
       (SELECT MAX(year) FROM season_entries) - MAX(r.year) AS years_since
FROM circuits c JOIN races r ON r.circuit_id = c.id
WHERE NOT EXISTS (SELECT 1 FROM races s WHERE s.circuit_id = c.id
                    AND s.year >= (SELECT MAX(year) FROM season_entries) - 1)
GROUP BY c.id
