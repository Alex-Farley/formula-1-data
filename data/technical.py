...
# that the clinching round and the title permutations are computable from
# `standings` and `points_systems` "once the per-season maximum-points rule is
# written down", and this is that rule written down. They are not a second
...
# points_systems after DA-18, and taken in the same pull request so that it is
# built on the corrected shape: `session` says whose grid a row sets - 'race'
...
# unless the source names the event the change came at, and verify.py holds
# that event's name to the round.
#
# `sessions` is how many qualifying sessions the weekend held for that grid;
# a knockout is one session run in three periods. `rule_107` is whether a
# driver outside 107% of the benchmark could be kept off the grid. The
# formats before 1996 are not here: the source describes them only as what
# was done "traditionally", with no first year, and known_gaps holds the span
# rather than a guess at it.
#
# Every row reads the one article below, which is a secondary source
# (confidence 'reference'); the 107% column is held to the FIA-read
# regulation_limits rows from 2018, and the knockout rows to the Q1/Q2/Q3
# times the qualifying sheets carry, both in verify.py.
_QUALI = "https://en.wikipedia.org/wiki/Formula_One_race_weekend"

# session, from_year, from_round, to_year, format, sessions, rule_107, note
QUALIFYING_FORMATS = [
    ("race", 1996, 1, 2002, "single session", 1, 1,
     "One session on Saturday afternoon, with twelve laps per driver; the "
     "Friday session was abolished. A driver outside 107% of the pole time "
     "did not qualify unless the stewards allowed it."),
    ("race", 2003, 1, 2003, "single lap", 2, 0,
     "Two sessions, on Friday and Saturday afternoon, each car running alone "
     "for one timed lap. Friday's times set Saturday's running order and not "
     "the grid, and Saturday's lap was run on race fuel."),
    ("race", 2004, 1, 2004, "single lap", 2, 0,
     "The Friday session moved to Saturday, so both ran that day; the first "
     "ran in the order of the previous race's result."),
    ("race", 2005, 1, 2005, "single lap, aggregate", 2, 0,
     "One lap on Saturday afternoon and one on Sunday morning, the grid "
     "being the aggregate of the two times."),
    ("race", 2005, 7, 2005, "single lap", 1, 0,
     "From the European Grand Prix the Sunday session was dropped: one lap "
     "each on Saturday afternoon, in the reverse order of the previous race's "
     "result."),
    ("race", 2006, 1, 2010, "knockout", 1, 0,
     "Three periods on Saturday afternoon, Q1, Q2 and Q3. The slowest cars "
     "are eliminated at the end of Q1 and of Q2, and Q3 sets the top ten."),
    ("race", 2011, 1, 2015, "knockout", 1, 1,
     "The 107% rule returned, measured against the fastest time in Q1."),
    ("race", 2016, 1, 2016, "knockout, elimination", 1, 1,
     "For the first two rounds drivers were eliminated during each period "
     "rather than at its end, and eight reached the last."),
    ("race", 2016, 3, 2020, "knockout", 1, 1,
     "The knockout returned from the Chinese Grand Prix. From 2018 a session "
     "declared wet is exempt from the 107% rule."),
    ("race", 2021, 1, 2022, "knockout", 1, 1,
     "At the sprint weekends qualifying moved to Friday and set the sprint's "
     "grid, and the sprint set the Grand Prix's."),
    ("race", 2023, 1, None, "knockout", 1, 1,
     "Qualifying sets the Grand Prix grid at every event again, sprint "
     "weekends included. From 2026, with 22 cars, six are eliminated in each "
     "of Q1 and Q2."),
    ("sprint", 2021, 1, 2022, "knockout", 1, 1,
     "No session of its own: the Grand Prix's qualifying, held on Friday, "
     "set the sprint's grid."),
    ("sprint", 2023, 1, 2023, "knockout", 1, 1,
     "The sprint shootout, on Saturday: three periods, SQ1, SQ2 and SQ3, of "
     "12, 10 and 8 minutes."),
    ("sprint", 2024, 1, None, "knockout", 1, 1,
     "Renamed sprint qualifying and moved to Friday afternoon, after the "
     "weekend's only practice session."),
]
...
GLOSSARY = [
    ...
    ("NC", "results", "Not classified. In a race, a driver who covered too little of the distance to be given a position - usually one still running at the end, a retirement being a DNF, though a source sometimes prints NC for a retirement too. In qualifying, a driver with no time that counts: none set, or one outside the 107% limit."),
    ("DNS", "results", "Did not start: entered and at the event but not in the start of the race - a car withdrawn or broken before the start, a driver hurt in practice, or a car handed to another driver."),
    ("DNQ", "results", "Did not qualify: an entry that did not earn a place on the grid - too slow when more cars were entered than were allowed to start, or outside the 107% limit - or that never had the chance, through an accident, an injury or a car taken away."),
    ...
    ("DNP", "results", "Did not practise: entered for the event but took part in none of its sessions, so the entry has no time and no result - even where the car itself ran, in another driver's hands."),
    ("DSQ", "results", "Disqualified: took part and was then removed from the result, usually for a technical or sporting breach found afterwards. In a championship table, a driver or team removed from the classification - Michael Schumacher in 1997, whose points stand without a position, and Tyrrell in 1984."),
    ...
    ("PL", "results", "Pit lane: in the Grid column, a driver who started the race from the pit lane rather than from a grid slot."),
    ...
    ("Shared drive", "results", "Two or more drivers taking turns in one car in the same race, which the championship's early seasons allowed. Each is credited with the car's result, so a classification can show one position more than once."),
